...
func _start_round() -> void:
	...
	target_rgb_label.text = "Alvo: (%d, %d, %d)" % [_c8(target.r), _c8(target.g), _c8(target.b)]
	...
	attempt_label.text = "Tentativa %d / %d" % [round_idx + 1, ROUND_COUNT]
	phase = Phase.MIX
...
func _on_submit() -> void:
	...
	feedback_body.text = "Diferenças por canal — R: %d, G: %d, B: %d. Tolerância: %d." % [dr, dg, db, TOLERANCE]
...
func _show_final() -> void:
	...
	if hits >= 1:
		final_title.text = "Trabalho aceito: %d / %d" % [hits, ROUND_COUNT]
		final_title.modulate = Color(0.4, 0.95, 0.4)
	else:
		final_title.text = "Trabalho recusado: %d / %d" % [hits, ROUND_COUNT]
		final_title.modulate = Color(1.0, 0.5, 0.4)

	final_body.text = "Ganhou +%d XP, perdeu -%d XP. Saldo: %+d XP." % [int(total_gain), int(total_loss), int(total_gain - total_loss)]
