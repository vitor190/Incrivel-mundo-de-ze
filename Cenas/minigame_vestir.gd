...
# Mesma curva narrativa do minigame_acordar: 3 tentativas com a sequência
# revelada por mais tempo (memorizar fica mais fácil a cada tentativa).
const ROUNDS := [
...
func _finish_success() -> void:
	...
	if total_penalty > 0:
		result_body.text = "Zé vestiu o uniforme depois de %d tropeço(s). Total perdido: −%d XP." % [int(total_penalty / WRONG_PENALTY), int(total_penalty)]
	else:
		result_body.text = "Zé vestiu o uniforme de primeira. Hora de ir pra aula."
