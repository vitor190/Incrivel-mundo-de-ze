...
const MINIGAME = preload("res://Cenas/minigames/CorrigirCodigo.tscn")
...
func _fechar_minigame(sucesso, xp):
	...
	PlayerScript.travado = false
	if sucesso:
		print("Ganhou %d XP!" % xp)
	# Removido o timer automático — agora espera o minigame se tornar invisível
